...
typedef cipher_t* (*CreatePlayfairFunc)(const char* key);
...
int main() {
    ...
    CreatePlayfairFunc create_playfair = (CreatePlayfairFunc)GetProcAddress(hLib, "cipher_create_playfair");
    ...
    while (running) {
        ...
        if (choice == 1) {
            ...
            std::cout << "\n----- Playfair Cipher -----" << std::endl;

            std::string key;
            std::cout << "Enter keyword (letters only): ";
            std::getline(std::cin, key);

            cipher_t* cipher = create_playfair(key.c_str());
            if (!cipher) {
                std::cerr << "Error creating Playfair cipher!" << std::endl;
                continue;
            }
